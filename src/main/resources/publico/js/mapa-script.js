...
async function cargarRegistrosEnMapa() {
    ...
    try {
        ...
        Object.keys(marcadores).forEach(key => delete marcadores[key]);
        ...
        registrosValidos.forEach((registro, index) => {
            ...
            marker.on("click", () => {
                ...
                desplazarAItem(markerId);
            });

            bounds.push([lat, lng]);
        });

        todosLosRegistros = [...registrosValidos].reverse();
        paginaActual = 1;
        renderizarLista();
...
function renderizarLista() {
    ...
    const limite = paginaActual * registrosPorPagina;
    const registrosAMostrar = todosLosRegistros.slice(0, limite);
    ...
    registrosAMostrar.forEach((registro, index) => {
        ...
        const markerId = registro.id || `registro-${todosLosRegistros.length - 1 - index}`;
        ...
        item.addEventListener("click", () => {
            const marker = marcadores[markerId];
            if (marker) {
    ...
    if (limite < todosLosRegistros.length) {
        const contenedorBoton = document.createElement("div");
        contenedorBoton.className = "mt-3";

        const botonVerMas = document.createElement("button");
        botonVerMas.type = "button";
        botonVerMas.className = "btn btn-outline-primary btn-sm w-100";
        botonVerMas.innerHTML = `<i class="bi bi-plus-circle me-1"></i> Ver más`;
        ...
        contenedorBoton.appendChild(botonVerMas);
...
function desplazarAItem(id) {
    const item = document.querySelector(`.item-registro[data-id="${id}"]`);
    if (item) {
        item.scrollIntoView({
            behavior: "smooth",
            block: "nearest"
